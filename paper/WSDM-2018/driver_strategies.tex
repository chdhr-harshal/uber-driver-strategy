...
\cumulativeearning{t}{i}{b} &=& \max_{a \in \actionsset}
    ...
    -cost^t(i,\homezone) + \cumulativeearning{t'}{\homezone}{b}, &\textrm{  if } a = \gohomeaction \\ \\
    \max_{j} \bigg\{-cost^t(i,j) + \cumulativeearning{t'}{j}{b'}\bigg\}, &\textrm{  if } a = \relocateaction
    \end{cases}
\end{eqnarray}
...
In the {\naive} strategy, a driver performs a random walk over the city from 9AM-5PM, with locations dictated exclusively by the 
passengers he picks up. At the end of every passenger ride, the driver waits in the current zone for next passenger pickup. 
Hence, the only allowable action is {\getpassenger}.


In the {\flexible} strategy, a driver has flexibility to decide working times, modeling a driver who uses heuristics to decide the most pprofitable times to work.   As a result, we impose an additional constraint of a working time budget $B$ that a driver can split over a finite horizon of $N$ time units. Thus, this strategy aims to figure out an optimal 
...
{\flexible} strategies can be done by streamlined versions of the dynamic programming recursion presented in 
