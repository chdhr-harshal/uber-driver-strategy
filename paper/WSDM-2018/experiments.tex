...
contains yellow street-hail records of over 65,000 daily taxi rides with fields capturing pickup and dropoff dates/times, 
...
$K<0$, there are more drivers than passengers in a given zone. We assume the worst case scenario in which a driver joins the end
 of the corresponding FIFO queue. Hence, for a state $k \leq 0$, the driver has to wait for $(|k| + 1)$ passenger arrivals in a unit time for a successful passenger pickup. The probability of a successful passenger pickup can be expressed as,
\begin{equation}
...
To respect the Uber rate limit of 1,000 API requests per hour per account, we subsampled one ride between each pair of zones in
...
significantly in 15 minutes. Chen {\etal}~\cite{chen2015peeking} have observed that 90\% of the surges on Uber platform 
...
Using this approach, we collected data from the Uber API for a period of 6 months from October 2016 to March 2017, 
recreating rides that originally occurred from 
October, 2015 until March, 2016.   From these recreated rides, we were able to build realistic estimates for $r(i,j)$ and $\tau(i,j)$
...
\begin{figure*}
	...
	\caption{Daily driver earnings with different strategies in a representative week.}
	\label{fig:daily_earnings}
\end{figure*}

\begin{figure}
	\includegraphics{figures/earnings_heatmap.pdf}
	\caption{Disparity of earnings between {\naive} and {\relocation} strategies on a representative day.}
...
\textit{Are the conclusions we drew above robust to perturbations of the empirical tansition matrices?} We address this question using
...
Figure \ref{fig:uncertainty_evolution} shows the results of increasing uncertainty on the earnings of drivers following each of the four strategies. We find that the {\relocation}, {\flexible} and {\relocationflexible} strategies are more tolerant to the uncertainty in the input transition matrices. Interestingly, even with 99\% uncertainty, the {\relocationflexible} strategy outperforms the {\naive} strategy with no uncertainty in form of \$100 extra earnings in a typical workday. This observation supports our claim that strategies based on historical data can improve driver earnings in the on-demand ridesharing platforms. 
