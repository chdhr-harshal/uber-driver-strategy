...
October 2015 to March 2016.   From these recreated rides, we were able to build realistic estimates for $r(i,j)$ and $\tau(i,j)$
...
that for a part-time Uber driver, it is much more beneficial to drive midweek from Wednesday to Friday, than during weekends.
...
Thus, we address the question: \textit{what is the best time of the day to drive in order to maximize earnings?} To answer this question, we simulate 1000 drivers, each randomly assigned a home zone, for each of the {\flexible} and {\relocationflexible} strategies. 
...
Thus, at every step of the simulation, a driver undertakes the personalized action recommended by the strategy, corresponding to
...
output by {\flexible} and {\relocationflexible}, respectively.
...
is, surprisingly, significantly higher than those by performed by the {\relocationflexible} strategy drivers. 
...
In fact, Uber prominently displays surge information to drivers on their Partner App,
  in the form of a heatmap of surge multipliers in different geographical neighborhoods.
...
Figure \ref{fig:simulated_earnings} reinforces our previous observation regarding high variance of the {\naive} strategy. 
...
may land a driver in a sub-optimal location with respect to longer term earnings maximization. 
