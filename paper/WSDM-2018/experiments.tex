...
Using the above input, we now evaluate our methodology. All the subsequent experiments were performed using a single process implementation of the dynamic programming algorithms on a 24-core 2.9GHz Intel Xeon E5 processor with 512GB memory. Computational overhead for the {\naive} and the {\relocation} strategies is less than a minute, and about 5 minutes for the {\flexible} and the {\relocationflexible} strategies. Uncertainty analysis (presented in Section \ref{sec:effect_of_uncertainty}), with an off-the-shelf minimizer, takes around 3 hours.
