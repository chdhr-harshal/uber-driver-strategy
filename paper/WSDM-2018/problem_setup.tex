...
\spara{\sc{CountMatrix}(\countmatrix)}: 
...
\spara{\sc{EmpiricalTransitionMatrix} (\empiricaltransitionmatrix)}:
...
\spara{\sc{TravelTimeMatrix} (\traveltimematrix)}:
...
\spara{\sc{RewardsMatrix} (\rewardsmatrix)}:
...
\spara{\sc{DriverActions} (\actionsset)}: 
...
\squishlist
	...
	\actionearning{i}{a_0} = \empiricaltransitionmatrix^t_{i}\bigcdot \rewardsmatrix^t_{i}
	\end{equation}
	...
	\actionearning{i}{a_1} = r^t(i,\homezone)
	\end{equation}
...
Therefore our optimization problem is to find the
driver policy \policy, such that the total \emph{total expected earnings}
of the driver is maximized. Thus the goal is to find $\policy^\ast$
such that:
\[
