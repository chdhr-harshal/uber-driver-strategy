...
We always assume that the driver starts from a home zone and returns to it
at the end of each of the shifts.
%\subsubsection{\sc{DriverActions} (\actionsset)}

\spara{Driver actions (\actionsset)}: 
In a driver strategy, whenever faced with a choice regarding next decision, a driver has $n+2$ possible actions to choose from: 
%\begin{itemize}
\squishlist
