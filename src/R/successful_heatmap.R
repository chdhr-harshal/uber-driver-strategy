...
# Read demands data
demand_df = data.table(read.delim("./plots_data/unsuccessful_heatmap.csv.gz", sep=","))

# Merge dataframes
dd = merge(city_zones_df, demand_df, by.x='id', by.y='zone', all.x=TRUE, allow.cartesian=TRUE)
...
p0 = ggmap(get_map(c(lon=-74.0, lat=40.74), scale="auto", zoom=10, maptype="roadmap", color="bw"), 
           extent="device")
p = p0 + facet_wrap(~time, nrow=2, ncol=2)
p = p + geom_polygon(aes(long, lat, group = group, fill=1 - unsuccessful),
                     data=dd,
                     alpha=0.8,
                     size=0.2,
                     colour="black")
p = p + scale_fill_gradientn(name="Probability\nof\nfinding\na passenger", colours=mycol)
...
p
ggsave(paste0(figures.dir, "successful_heatmap.pdf"), w=8, h=8, units="cm")
